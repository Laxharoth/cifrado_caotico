...
double aa1;
double f0(double t) { return pow(t, aa1) * exp(-t); }
...
double chi2UniformDistance(double *ds, int dslen) {
    ...
    for (k = 0; k < dslen; k++) expected += ds[k];
    expected /= k;
...
double chiIsUniform(double *dset, int dslen, double significance) {
    int dof = dslen - 1;
    double dist = chi2UniformDistance(dset, dslen);
    return chi2Probability(dof, dist);
}
